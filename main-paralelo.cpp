...
void AvaliacaoParalela(vector<int> individuo, vector<pair<int, int>> valoresPesos, int &valorTotal){
  mutex m;
  for (auto i : individuo) {
  m.lock(); // Bloqueia a thread pra evitar que acessem a variável valorTotal
...
int Avaliacao(vector<int> individuo, vector<pair<int, int>> valoresPesos) {
  ...
  for(int i = 0, j = 0; i < numThreads; i++, j += ((dados.numItens/numThreads) - 1)){
    // Faz a divisão do indivíduo em partes iguais para cada thread
    vector<int> individuoAux(individuo.begin() + j, individuo.begin() + j + (dados.numItens/numThreads));
    // Cria thread para cada parte do indivíduo
    threads.emplace_back(AvaliacaoParalela, individuoAux, valoresPesos, ref(valorTotal));
  }
